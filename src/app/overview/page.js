...
export default function OverviewPage() {
  ...
  const toggleWorkDetails = () => setShowWorkDetails((prev) => !prev);
  // 👉 Work Details Data
  const workDetails = [
        ...
        <div className="w-full p-6 bg-white space-y-6 border-b border-gray-200">
          ...
          <div className="space-y-6">
            {activityLogs.map((log, index) => (
              <div
                key={index}
                className="space-y-1 border-b border-gray-200 p-2"
              >
                <p
                  className={`text-sm font-semibold ${getTitleColor(log.type)}`}
                >
                  {log.title}{" "}
